...
dataDIR = '/lou/la2/vhartwic/FV3/verona/fv3_2.0_windenergy/fms_mars_MY24_highres/history/MY24_highres_solarwindpercent2500_altturb.nc'
...
aep_Jac = pow_tot_diurn_Jac/1000*5  #kWh
...
aae_ave_Jac = aae_Jac * 5 * len(DDS_MY24.time) 

pow_tot_diurn_NREL = np.sum(DDS_MY24.pow_diurn_NREL, axis=1) # sum over time of day
energy_per_sol_NREL = pow_tot_diurn_NREL.mean('time')
pow_tot_diurn_NREL = np.sum(pow_tot_diurn_NREL, axis=0)
aep_NREL = pow_tot_diurn_NREL*5*1e-6 #GWh

pow_ave_NREL = np.mean(DDS_MY24.pow_diurn_NREL, axis=0)
aae_NREL = np.mean(pow_ave_NREL, axis=0)
aae_ave_NREL = aae_NREL * 5 * len(DDS_MY24.time) 

# print global average AEP
weights = np.cos(np.deg2rad(DDS_MY24.lat))
weights.name = "weights"
print('global average JAC, NRELaep=', aep_Jac.weighted(weights).mean(('lon','lat')), aep_NREL.weighted(weights).mean(('lon','lat')))

#)e Equivalent Solar Panel Array Area
...
# Jacobs
fig = ax[0].contourf(DDS_MY24.lon, DDS_MY24.lat, aep_Jac[:,:], 20, cmap=py.cm.viridis)
...
fig = ax[1].contourf(DDS_MY24.lon, DDS_MY24.lat,DDS_ALT.percent_Jac*100-DDS_PCT.percent_solar2500*100,cmap=py.cm.magma)
...
# NREL
fig = ax[3].contourf(DDS_MY24.lon, DDS_MY24.lat, aep_NREL[:,:], 20, cmap=py.cm.viridis)
...
fig = ax[4].contourf(DDS_MY24.lon, DDS_MY24.lat,DDS_ALT.percent_NREL*100-DDS_PCT.percent_solar2500*100,cmap=py.cm.magma)
...
ax[5].plot(lon_sav15_NREL, lat_sav15_NREL, 'vk', markersize=2, markeredgecolor='blue',markerfacecolor='blue', label='>15kW')
...
# Save Data
var1 = xr.DataArray(aep_Jac, coords={'lat':DDS.lat, 'lon':DDS.lon},dims=['lat','lon'])
var2 = xr.DataArray(aep_NREL, coords={'lat':DDS.lat, 'lon':DDS.lon},dims=['lat','lon'])

NEW_DF = xr.Dataset({'aep_Jac':aep_Jac,'percent_Jac':DDS_ALT.percent_Jac,'percent_solar2500':DDS_PCT.percent_solar2500,
	'zsurf':DDS.zsurf,'lon_sav15_JAC':lon_sav15_JAC,'lat_sav15_JAC':lat_sav15_JAC,'lon_sav24_JAC':lon_sav24_JAC,
	'lat_sav24_JAC':lat_sav24_JAC,'aep_NREL':var2,'percent_NREL':DDS_ALT.percent_NREL,'lon_sav15_NREL':lon_sav15_NREL,
	'lat_sav15_NREL':lat_sav15_NREL,'lon_sav24_NREL':lon_sav24_NREL,'lat_sav24_NREL':lat_sav24_NREL})
