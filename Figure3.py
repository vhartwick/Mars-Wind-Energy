...
#WPD
wpd_season_MY28 += [DDS_MY28.wpd[ls90,:,:,:,:].mean(('time','time_of_day_24')).rename('wpd_Ls90')]
wpd_season_MY28 += [DDS_MY28.wpd[ls180,:,:,:,:].mean(('time','time_of_day_24')).rename('wpd_Ls180')]
wpd_season_MY28 += [DDS_MY28.wpd[ls270,:,:,:,:].mean(('time','time_of_day_24')).rename('wpd_Ls270')]
wpd_season_MY28 += [DDS_MY28.wpd[ls0,:,:,:,:].mean(('time','time_of_day_24')).rename('wpd_Ls0')]
wpd_MY28 = xr.merge(wpd_season_MY28)

# SOLAR
swflx_season_MY28 += [DDS_MY28.swflx[ls90,:,:,:].mean(('time','time_of_day_24')).rename('swflx_Ls90')]
swflx_season_MY28 += [DDS_MY28.swflx[ls180,:,:,:].mean(('time','time_of_day_24')).rename('swflx_Ls180')]
swflx_season_MY28 += [DDS_MY28.swflx[ls270,:,:,:].mean(('time','time_of_day_24')).rename('swflx_Ls270')]
swflx_season_MY28 += [DDS_MY28.swflx[ls0,:,:,:].mean(('time','time_of_day_24')).rename('swflx_Ls0')]
swflx_MY28 = xr.merge(swflx_season_MY28)

# DUST OPTICAL DEPTH
taudust_season_MY28 += [DDS_MY28.taudust_VIS[ls90,:,:,:].mean(('time','time_of_day_24')).rename('taudust_Ls90')]
taudust_season_MY28 += [DDS_MY28.taudust_VIS[ls180,:,:,:].mean(('time','time_of_day_24')).rename('taudust_Ls180')]
taudust_season_MY28 += [DDS_MY28.taudust_VIS[ls270,:,:,:].mean(('time','time_of_day_24')).rename('taudust_Ls270')]
taudust_season_MY28 += [DDS_MY28.taudust_VIS[ls0,:,:,:].mean(('time','time_of_day_24')).rename('taudust_Ls0')]
...
print('time',DDS_MY24.time[ls270])
# average all years to produce multiyear seasonal averages (MC_season)
wpd_season_MY24 = []
swflx_season_MY24 = []
taudust_season_MY24 = []
...
clb.set_label('$\ [W/m^{2}$]')
print(max(wpd_MY28.wpd_Ls270.sel(zagl=50).values-wpd_MY24.wpd_Ls270.sel(zagl=50).values,keepdims=True))
print(min(swflx_MY28.swflx_Ls270.values-swflx_MY24.swflx_Ls270.values,keepdims=False))
...
# print approximate percent decrease in global solar power
weights = np.cos(np.deg2rad(DDS_MY24.lat))
weights.name = "weights"
tmp24 = swflx_MY24.swflx_Ls270.weighted(weights).mean(('lon','lat'))
tmp28 = swflx_MY28.swflx_Ls270.weighted(weights).mean(('lon','lat'))
percentdiff = (tmp28-tmp24)/tmp24 * 100
#print(tmp24, tmp28, percentdiff)

py.savefig(f'{PATH}/WindEnergy_HighRes_Fig{ct}.eps',dpi=300)

# Send out Data
var1 = xr.DataArray(wpd_MY24.wpd_Ls270.sel(zagl=50), coords={'lon':DDS_MY24.lon, 'lat':DDS_MY24.lat},dims=['lat','lon'])
var1.name = 'wpd_MY24_Ls270'

var2 = xr.DataArray(wpd_MY28.wpd_Ls270.sel(zagl=50), coords={'lon':DDS_MY24.lon, 'lat':DDS_MY24.lat},dims=['lat','lon'])
var2.name = 'wpd_MY28_Ls270'

var3 = xr.DataArray(swflx_MY24.swflx_Ls270, coords={'lon':DDS_MY24.lon, 'lat':DDS_MY24.lat},dims=['lat','lon'])
var3.name = 'swflx_MY24_Ls270'

var4 = xr.DataArray(swflx_MY28.swflx_Ls270, coords={'lon':DDS_MY24.lon, 'lat':DDS_MY24.lat},dims=['lat','lon'])
var4.name = 'swflx_MY28_Ls270'

var5 = xr.DataArray(taudust_MY24.taudust_Ls270, coords={'lon':DDS_MY24.lon, 'lat':DDS_MY24.lat},dims=['lat','lon'])
var5.name = 'taudust_MY24_Ls270'

var6 = xr.DataArray(taudust_MY28.taudust_Ls270, coords={'lon':DDS_MY24.lon, 'lat':DDS_MY24.lat},dims=['lat','lon'])
var6.name = 'taudust_MY28_Ls270'
