...
# Bin Annual Average Wind Speeds
...
for x,y in zip(bins, bins_top):   # loop through wind speed bins
    tmp = np.count_nonzero(np.where(np.logical_and(DDS_MY24.veladj.sel(zagl=50, method='nearest') >=x,
                                                   DDS_MY24.veladj.sel(zagl=50, method='nearest')  < y)))
    tmp2 = np.count_nonzero(np.where(np.logical_and(DDS_MY24.ws.sel(zagl=50, method='nearest') >=x,
                                                    DDS_MY24.ws.sel(zagl=50, method='nearest') < y)))
...
fig1 = ax1.bar(bins, percent_time2_SY1*100, width=0.5,color='white', edgecolor='black')
fig1 = ax1.bar(bins,percent_time_SY1*100, width=0.5,hatch='/', color='grey', edgecolor='black',
        bottom=percent_time2_SY1*100)
...
# Send out Data
var1 = xr.DataArray(percent_time_SY1)
var1.name = 'percent_time_SY1'

var2 = xr.DataArray(percent_time2_SY1)
var2.name = 'percent_time2_SY1'
